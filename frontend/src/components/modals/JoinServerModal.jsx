...
export default function JoinServerModal({ onClose, onJoined }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.success(`Vous avez rejoint ${data.name}`);
      onJoined(data); onClose();
    } catch (e) { toast.error(e?.response?.data?.detail || "Invitation invalide"); }
    finally { setLoading(false); }
  };
  return (
    <ModalShell title="Rejoindre un serveur" onClose={onClose} testId="join-server-modal">
      <h2 className="font-display text-3xl font-extrabold uppercase tracking-tighter mt-2">Vous avez une invitation ?</h2>
      <p className="text-cc-subtext text-sm mt-2">Collez un code d'invitation ou un lien complet ci-dessous.</p>
