  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-12 bg-cc-base text-cc-text">
      ...
      <div className="lg:col-span-5 flex flex-col px-8 lg:px-14 py-10">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-md w-full mx-auto">
          ...
          <h1 className="font-display text-4xl lg:text-5xl font-extrabold tracking-tighter uppercase mt-4 leading-[0.95]">
            {mode === "login" ? <>Welcome<br/>back.</> : <>Make it<br/><span className="text-cc-accent">yours.</span></>}
          </h1>
          ...
          <form onSubmit={onSubmit} className="mt-10 space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 border-b border-cc-border focus-within:border-cc-accent transition-colors mt-2">
                  ...
                  <input
                    ...
                    placeholder="Vector"
                    className="flex-1 bg-transparent py-3 outline-none placeholder:text-cc-muted"
                    required
                  />
                </div>
              </div>
            ...
            <div>
              <label className="text-[10px] tracking-[0.3em] font-bold text-cc-muted uppercase">Email</label>
              <div className="flex items-center gap-3 border-b border-cc-border focus-within:border-cc-accent transition-colors mt-2">
            ...
            <div>
              <label className="text-[10px] tracking-[0.3em] font-bold text-cc-muted uppercase">Password</label>
              <div className="flex items-center gap-3 border-b border-cc-border focus-within:border-cc-accent transition-colors mt-2">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="flex-1 bg-transparent py-3 outline-none placeholder:text-cc-muted font-jetbrains"
                  minLength={8}
                  required
                />
              </div>
              {mode === "register" && <p className="text-[10px] text-cc-muted mt-2 uppercase tracking-widest">Min 8 chars · stored bcrypt-hashed</p>}
            </div>

            {error && (
              <div data-testid="auth-error" className="text-cc-danger text-sm border-l-2 border-cc-danger pl-3">
                {error}
              </div>
            )}
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-cc-accent text-white font-bold tracking-wide uppercase py-4 cc-brutal-shadow cc-brutal-press disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? <div className="cc-spinner" /> : <>{mode === "login" ? "Authenticate" : "Create account"} <ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>

          <div className="my-8 flex items-center gap-4">
            <div className="flex-1 h-px bg-cc-border" />
            <span className="text-[10px] tracking-[0.3em] uppercase text-cc-muted">or</span>
            <div className="flex-1 h-px bg-cc-border" />
          </div>
          ...
          <button
            ...
            type="button"
            onClick={googleLogin}
            className="w-full border border-cc-border bg-cc-surface1 hover:bg-cc-surface2 font-bold uppercase tracking-wide py-4 transition-colors flex items-center justify-center gap-3"
          >
          ...
          <p className="text-center text-cc-subtext text-sm mt-8">
            ...
            <button
              ...
              className="text-cc-accent font-bold uppercase tracking-wider"
            >
      ...
      <motion.div
        ...
        className="hidden lg:block lg:col-span-7 relative bg-cc-surface1 border-l border-cc-border overflow-hidden"
      >
        <img
          alt=""
          src="https://static.prod-images.emergentagent.com/jobs/a5c2efcc-de7a-4769-b97b-5f02393024a7/images/93de6890741f329a6c03ab059fbad0b4325d61e957e4cae8da90ea78e9fd9274.png"
        ...
        <div className="absolute bottom-12 left-12 right-12 max-w-lg">
          <span className="text-xs uppercase tracking-[0.3em] text-cc-muted font-bold">
            <span className="inline-block w-3 h-3 bg-cc-accent mr-3 align-middle" /> Inside the build
          </span>
          <h2 className="font-display text-3xl lg:text-5xl font-extrabold tracking-tighter uppercase mt-5 leading-[0.95]">
            Where signal<br/>meets <span className="text-cc-accent">privacy.</span>
          </h2>
          <p className="text-cc-subtext mt-6">
            CentCord pairs the joy of community chat with cryptographic discipline. Servers, voice, threads, friends — defended by default.
          </p>
        </div>
      </motion.div>
    </div>
